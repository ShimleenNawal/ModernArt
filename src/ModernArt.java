...
public class ModernArt {
    ...
    public void prepareDeck() {
        //TODO
        for (int artist = 0; artist < PAINTS.length; artist++) {
            for (int type = 0; type < PAINTS[artist].length; type++) {
                for (int count = 0; count < PAINTS[artist][type]; count++) {
                    if (type == 0)
                        deck.add(new OpenAuctionPainting(artist));
                    else if (type == 1)
                        deck.add(new HiddenAuctionPainting(artist));
                    else if (type == 2)
                        deck.add(new OneOfferAuctionPainting(artist));
                    else if (type == 3)
                        deck.add(new FixedPriceAuctionPainting(artist));
                }
            }
        }
    ...
    public void dealPainting(int round) {
        ...
        for (int i = 0; i < players.length; i++) {
            for (int j = 0; j < paintingsToDeal; j++) {
                if (!deck.isEmpty()) {
                    Painting painting = deck.remove(0);
                    players[i].dealPaintings(painting);
                    painting.setOwner(players[i]);
                }
            }
        }
    }
