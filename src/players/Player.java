...
public class Player {
    ...
    public Painting playPainting() {
        ...
        while (true) {
            ...
            for (int y = 0; y < handPaintings.size(); y++) {
                System.out.println(y + ": " + handPaintings.get(y));
            }

            System.out.print("Please enter the index of the Painting you want to play: ");

            if (Scanner.hasNextInt()) {
                paintingIndex = Scanner.nextInt();
                if (paintingIndex >= 0 && paintingIndex < handPaintings.size()) {
                    return handPaintings.remove(paintingIndex);
                }
            } else {
    ...
    public void pay(int amount) {
        ...
        if (amount <= money) {
            ...
            throw new IllegalArgumentException("Not enough money to pay this amount.");
        }
    }
    ...
    public void sellPainting(int[] scores) {
        ...
        for (Painting painting : boughtPaintings) {
            ...
            if (scores[artistId] > 0) {
                money += scores[artistId];
            }
        }
